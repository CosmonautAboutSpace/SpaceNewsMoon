...
def improved_fake_score(text: str, title: str = "") -> float:
    ...
    if words:
        ...
        score += min(ratio * 120, 25)
    avg_len = sum(len(w) for w in words) / max(1, len(words)) if words else 0
    if len(combined) < 280:
        score += 6
    if avg_len < 4.0:
        score += 5
...
<html lang="ru">
<head>{{ head|safe }}</head>
<body style="background:#0b1020;color:#e8eeff;font-family:Inter, sans-serif; padding:20px;">
<header>
  <h1>О нас</h1>
</header>
<main>
  <div>
    <p>Этот сайт был разработан любителем космоса и астрономии.</p>
    <p>Здесь вы можете публиковать новости, смотреть фазу Луны и изучать космос с помощью 3D-модели на главной странице.</p>
    <p>Подписывайтесь на наш Telegram: <a href="https://t.me/SpaceNewsMoon" target="_blank" style="color:#6aa7ff">https://t.me/SpaceNewsMoon</a></p>
  </div>
</main>
<footer>
...
def add_news():
    ...
    if audio_file and audio_file.filename:
        ...
        if aext in ALLOWED_AUDIO:
            ...
            audio_file.save(os.path.join(app.config['UPLOAD_FOLDER'], afname))
            audio_name = afname
        else:
